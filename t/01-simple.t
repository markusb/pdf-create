...
if (-x '/usr/bin/pdftotext') {
  if (my $out=`/usr/bin/pdftotext $pdfname -`) {
    print "ok 5 # pdf reads fine with pdftotext\n";
  } else {
    print "not ok 5 # pdftotext reported errors\n";
    exit 1;
  }
} else {
  print "ok 5 # Warning: /usr/bin/pdftotext not installed";
}
print "ok 6 \# test $0 ended\n";
